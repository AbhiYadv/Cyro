...
| CYRO-0011A | Streaming + Cancel Contract | task/CYRO-0011A-streaming-cancel-contract | Ready for Review |  | 2026-05-16 | Docs/contracts only; locks Rust-owned `llama-cli` stdout streaming, `cancel_generation`, timeout cleanup, partial output, and one-active-generation policy |
| CYRO-0011B | Streaming stdout implementation | Phase 1B | Planned |  |  | Implements Tauri event or equivalent Rust-owned stdout streaming, cancel/kill cleanup, ordered partial output, and tests required by CYRO-0011A |
