...
| CYRO-0008 | Runtime Status/Error UX and Manual Sidecar Path Flow | task/CYRO-0008-runtime-status-path-flow | Ready for Review |  | 2026-05-15 | Adds manual sidecar/model path validation UI, combined runtime status, and visible `local_mock` vs `local_sidecar` route metadata; no streaming or downloads |
...
| CYRO-0010 | Streaming, Cancel, and Runtime Error UX Contract | Phase 1B | Planned |  |  | Defines token streaming, cancellation, partial output, process kill strategy, and richer process lifecycle UX |
