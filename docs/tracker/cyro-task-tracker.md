...
| CYRO-PROVIDER-010 | Tauri-native Provider Webview Container Research | task/CYRO-PROVIDER-010-tauri-native-webview-container | Ready for Review |  | 2026-05-19 | Adds visible Tauri WebviewWindow spike opened by hardcoded provider id only; ChatGPT/Claude/Gemini loaded visible logged-out provider-owned surfaces on macOS; login/chat/persistent sessions/in-layout child webview remain unvalidated; no provider APIs, scraping, DOM reading, JS injection, cookies, auto-login, prompt automation, response capture, memory import, or bypass |
