...
| CYRO-PROVIDER-009 | Provider Shell Chat UX Prototype | task/CYRO-PROVIDER-009-provider-shell-chat-ux | In Progress |  | 2026-05-18 | Builds modern Cyro-owned provider shell UX with drawer, provider header, bottom composer, tools tray, segmented Local/ChatGPT/Claude/Gemini provider pills, Fast/Think/Pro reasoning pills, Send/Stop, compact diagnostics, polished blocked state, and minimal Cyro Presence home state; provider routes are shell prototypes only and must show blocked/unvalidated/container-pending status; provider browser is not marked working; no provider APIs, scraping, DOM reading, cookies, auto-login, prompt automation, response capture, memory import, or bypass |
| CYRO-PROVIDER-010 | Tauri-native Provider Webview Container Research | Backlog | Planned |  |  | Research Tauri child webview vs WebviewWindow for visible in-Cyro provider origins with isolated sessions, navigation bounds, blocked-state handling, explicit fallback, and no provider APIs/scraping/DOM reading/cookie capture/auto-login/prompt automation/response capture |
| CYRO-PROVIDER-011 | Unified Composer Provider Routing | Backlog | Planned |  |  | Connects composer route selection to approved provider-shell/container state after CYRO-PROVIDER-010 research; no prompt injection or provider automation |
...
CYRO-PROVIDER-009 builds the modern Cyro-owned provider shell chat UX around that finding: header, drawer, tools tray, bottom composer, segmented provider pills, reasoning pills, Send/Stop, compact diagnostics, an honest blocked state, and a minimal composer-first Cyro Presence home state. It is not a provider automation or webview-container task and does not mark provider browser hosting as working. Provider shell is not validated yet. Gemini is available as a route in the shell prototype, but manual embedded-session validation is still pending. Iframe embedding remains a feasibility result, not the final provider-shell solution. CYRO-PROVIDER-010 must validate a Tauri-native visible webview/session container with no DOM, cookie, credential, prompt, or response capture.
