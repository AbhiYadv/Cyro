...
| CYRO-0011B-CANCEL-VALIDATION-HOOK | Dev-only cancel validation hook | task/CYRO-0011B-streaming-cancel-implementation | Ready for Review |  | 2026-05-17 | Adds explicit `CYRO_STREAM_TEST_SLOW=1` native dev validation mode so tiny proof models expose a Cancel window; normal runtime has no artificial stream delay |
