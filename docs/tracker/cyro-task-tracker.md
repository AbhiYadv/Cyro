...
CYRO-PROVIDER-009 builds the modern Cyro-owned provider shell chat UX around that finding: header, drawer, tools tray, bottom composer, segmented provider pills, reasoning pills, Send/Stop, compact diagnostics, and an honest blocked state. It is not a provider automation or webview-container task and does not mark provider browser hosting as working. The next container task is CYRO-PROVIDER-010 Tauri-native Provider Webview Container Research.
