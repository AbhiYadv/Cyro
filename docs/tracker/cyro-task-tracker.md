...
| CYRO-PROVIDER-011 | In-layout Native Provider Webview Research | task/CYRO-PROVIDER-011-in-layout-native-webview | Ready for Review |  | 2026-05-19 | Adds Tauri child webview prototype attached to the main Cyro window by hardcoded provider id only; manual native provider rendering validation is pending because `pnpm tauri dev` launch was blocked in this run; separate WebviewWindow remains fallback only; no provider APIs, scraping, DOM reading, JS injection, cookies, auto-login, prompt automation, response capture, memory import, or bypass |
...
CYRO-PROVIDER-011 adds the first in-layout Tauri child webview research spike. It uses Tauri's child webview path from the main Cyro window and keeps React limited to provider id requests. Rust maps provider ids to hardcoded origins, rejects arbitrary URLs, opens an incognito visible child webview, denies popups, and restricts navigation to the allowlisted provider host. Manual native validation is still pending because the environment rejected launching `pnpm tauri dev` in this run, so no provider route is marked as successfully in-layout yet. Separate WebviewWindow remains fallback only and is not final product UX.
