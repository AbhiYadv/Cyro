...
| CYRO-PROVIDER-012C | Provider Workspace UX Redesign | task/CYRO-PROVIDER-012-auto-inlayout-provider-tabs | In Progress |  | 2026-05-19 | UI-only polish: removes debug-like header/status clutter, keeps provider tabs calm with only active route state, upgrades provider pending copy, keeps provider-route composer disabled as prompt bridge pending, and preserves no provider APIs, scraping, DOM reading, JS injection, cookies, prompt automation, response capture, memory import, or bypass |
...
CYRO-PROVIDER-012C refines the visible Provider Workspace shell only. It reduces debug/status clutter, keeps the header to Cyro, active provider, and diagnostics, shows provider route state only on the active tab, and changes provider pending cards to product-grade Open-in-Cyro states while retaining the strict boundary that Cyro cannot read provider DOM, cookies, credentials, prompts, or responses. It does not change Rust/Tauri provider creation, provider allowlists, login/session validation, or prompt bridging.
